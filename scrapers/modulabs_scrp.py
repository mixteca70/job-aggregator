...
BASE_URL = "https://modulabs.career.greetinghr.com"
APPLY_URL = f"{BASE_URL}/en/apply"
SOURCE = "modulabs"


def fetch() -> list[dict]:
    """모두의연구소 채용 페이지에서 공고 수집."""
    jobs = []
    ...
    for a in soup.select('a[href*="/en/o/"]'):
        ...
        if "greetinghr.com" not in link or "/en/o/" not in link:
            continue
        if link in seen:
            continue
        seen.add(link)
