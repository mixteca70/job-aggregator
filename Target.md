...
## MVP vs Phase 2 구분 (1차 수집 범위)

| 구분 | 수집 방식 | 대상 | API 키 | 구현 상태 |
|------|-----------|------|--------|-----------|
| **MVP** | 스크래핑만 | SW중심대학 | 불필요 | ✅ swuniv_scrp |
| **MVP** | 스크래핑만 | 코드잇 | 불필요 | ✅ codeit_scrp |
| **MVP** | 스크래핑만 | 멋쟁이사자처럼 | 불필요 | ✅ likelion_scrp |
| **MVP** | 스크래핑만 | 모두의연구소 | 불필요 | ✅ modulabs_scrp |
| **MVP** | 스크래핑만 | 패스트캠퍼스 | 불필요 | ⚠️ fastcampus_scrp (구조 탐색) |
| **MVP** | 스크래핑만 | 엘리스 | 불필요 | ⚠️ elice_scrp (JS 렌더링) |
| **MVP** | 스크래핑만 | 커리어리 | 불필요 | ⚠️ careerly_scrp (전용 페이지 미확인) |
| **Phase 2** | API | 사람인, 원티드, 워크넷, 나라장터 | 필요 | 예정 |

MVP는 API 승인 대기 없이 프론트엔드-백엔드-DB 연동을 먼저 검증합니다.
...
| 구현 | `scrapers/swuniv_scrp.py` — BeautifulSoup + requests, html.parser |
| 선택자 | `a[href*='bmode=view']`, `a[href*='idx=']`, `.board-list a` 등 순차 시도 |
| 후처리 | parse_deadline, clean_html, match_keywords (KEYWORDS 8개) |
| 비고 | 각 선정대학 SW중심대학사업단 직원/강사 채용 공고. User-Agent, 2초 sleep. |
...
| 사이트 | URL | API | 수집 방법 | 구현 |
|--------|-----|-----|-----------|------|
| 패스트캠퍼스 | https://fastcampus.co.kr | X | HTML 스크래핑 | fastcampus_scrp (b2b.fastcampus.co.kr) |
| 엘리스 (Elice) | https://elice.io | X | HTML 스크래핑 | elice_scrp (elice.careers/jobs) |
| 멋쟁이사자처럼 | https://likelion.net | X | HTML 스크래핑 | likelion_scrp (likelion.career.greetinghr.com/ko/apply) |
| 코드잇 (Codeit) | https://www.codeit.kr | X | HTML 스크래핑 | codeit_scrp (careers.codeit.com/en/recruit) |
...
|--------|-----|-----|-----------|------|
| 커리어리 (Careerly) | https://careerly.co.kr | X | HTML 스크래핑 | careerly_scrp (전용 채용 페이지 미확인) |
| 모두의연구소 | https://modulabs.career.greetinghr.com | X | HTML 스크래핑 | modulabs_scrp (en/apply) |
...
| 수집 방법 | 적용 대상 | MVP/Phase2 | 구현 |
|-----------|-----------|------------|------|
| **HTML 스크래핑** | SW중심대학 | **MVP** | ✅ swuniv_scrp |
| **HTML 스크래핑** | 코드잇 | **MVP** | ✅ codeit_scrp |
| **HTML 스크래핑** | 멋쟁이사자처럼 | **MVP** | ✅ likelion_scrp |
| **HTML 스크래핑** | 모두의연구소 | **MVP** | ✅ modulabs_scrp |
| **HTML 스크래핑** | 패스트캠퍼스, 엘리스, 커리어리 | **MVP** | ⚠️ 구현됨 (구조/JS 이슈로 수집량 적음) |
| **API** | 사람인, 원티드, 워크넷, 나라장터 | Phase 2 | 예정 |
| **API (신청 제한)** | 잡코리아 | Phase 2 | 예정 |
